...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    // Fetch user's name and email from session and update profileData
    if (session && session.user) {
      ...
      // Fetch user's favorite fonts
      axios.get(`http://localhost:3001/api/user-favorites/${session.user.email}`) // <-- Corrected URL
        .then(response => {
  ...
  return (
    <div className="bg-black text-white min-h-screen">
      <div className="container flex-col flex items-center justify-center text-center mx-auto sm:w-full lg:w-2/3 pt-12 p-12">
        <div className="mb-8">
          <UserIcon />
        </div>
  ...
  {favorites.length === 0 ? (
    <p>No favorite fonts found.</p> // Display message when there are no favorite fonts
  ) : (
    <ul>
      {favorites.map((favorite, index) => (
        <li key={index} className="flex justify-between items-center border-2 border-purpur hover:border-white hover:bg-white hover:text-black text-purpur rounded-md px-4 py-2 mb-4">
          ...
          <button
            ...
            onClick={() => removeFavorite(index, favorite)} // Pass both index and fontFamily
          >
            <DeleteIcon />
          </button>
