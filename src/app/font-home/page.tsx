...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    const fetchFontFamily = async () => {
      try {
        const response = await axios.get(`http://localhost:3001/api/font-family?search=${search}`);
    ...
    <div id="top" className=" bg-black text-white min-h-screen pb-60" >
      ...
      <div className="text-white text-7xl md:text-8xl lg:text-9xl mb-10" style={{ fontFamily: `${fontUrls?.fontFamily}, sans-serif` }}>{search}</div>
        <div className="flex justify-center items-center">
          ...
          <button className="text-xl hover:text-purpur hover:border-purpur border-2 px-4 h-12 rounded-full border-white text-white" onClick={() => handleDownload(fontUrls.ttfUrl)}>
  Download
</button>
        </div>
      </div>
